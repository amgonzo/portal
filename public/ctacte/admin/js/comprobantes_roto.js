...
function cargarComboPersonas(incluirInactivos = true) {

    const todos = incluirInactivos ? 1 : 0;

    fetch(
        API_BASE + `/ctacte/personas/obtener_personas_combo.php?todos=${todos}`,
        {
            method: 'GET',
            headers: {
                'Authorization': 'Bearer ' + localStorage.getItem('sso_token')
        ...
        .then(personas => {
            let optionsModal = `
                ...
                <option value="0">
                    ⚠️ Devolver a Sin Usuario (DNI 0)
                </option>
            `;
            let optionsFiltro = `
                <option value="0" selected>
                    Solo Compras Sin Usuario (DNI 0)
                </option>
                <option value="todos">
                    -- Todos los empleados --
                </option>
            ...
            if (Array.isArray(personas)) {
                personas.forEach(p => {
                    ...
                    `;
                    optionsModal += opt;
                    // En el filtro superior solamente activos
                    if (esActivo) {
                        optionsFiltro += opt;
                    }
                });
            ...
            document.getElementById(
                'select_dni_nuevo'
            ).innerHTML = optionsModal;
            document.getElementById(
                'filtro_empleado'
            ).innerHTML = optionsFiltro;
        })
        .catch(err => {
            console.error(
...
function cargarFacturas() {
    ...
    params.set('anulados', verAnulados);

    fetch(
        API_BASE + `/ctacte/compras/obtener_compras_filtradas.php?${params.toString()}`,
        {
            method: 'GET',
            headers: {
                'Authorization': 'Bearer ' + localStorage.getItem('sso_token')

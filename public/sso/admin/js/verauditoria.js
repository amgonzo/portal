...
function verMas(index) {
    ...
    // Validar si el idregistro es nulo, cero, vacío o representa un valor inválido/masivo
    if (!log.idregistro || log.idregistro == 0 || log.idregistro === 'null' || log.idregistro === '#null') {
        $('#detCambios').html(renderCambios(log.dataantes, log.datadespues));
        $('#detActual').html('<span class="text-muted">Este registro corresponde a un evento global, error de sistema o acción masiva (sin ID de entidad individual).</span>');
        $('#modalDetalleLog').modal('show');
        return; // Detiene la ejecución para evitar el llamado AJAX con error
    }

    $('#detActual').html('<span class="text-muted">Cargando estado actual...</span>');

    $.ajax({
        url: API_BASE + '/sso/auditoria/get_registros.php', // Ajustado a ctacte tal como estabas usando
        type: 'POST',
        data: {
            tabla: log.tablename,
            id: log.idregistro

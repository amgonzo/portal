...
try {
    ...
    if (empty($motivo)) {
        throw new Exception("Debe ingresar un motivo para la anulación.");
    }
    ...
    if (isset($datosAntes['anulado']) && intval($datosAntes['anulado']) === 1) {
        throw new Exception("El comprobante ya se encuentra anulado.");
    }

    // 2. Validar cierre de período del empleado
    ...
    if ($res_c && intval($res_c['cerrado']) === 1) {
        throw new Exception("No se puede anular: el período correspondiente se encuentra cerrado.");
    }

    // 3. Marcar comprobante como anulado
    $stmt_u = $mysqli->prepare("
        UPDATE compras_cabecera 
        SET anulado = 1, motivo_anulacion = ? 
        WHERE punto_venta_id = ? AND venta_id = ?
    ");
    $stmt_u->bind_param("sii", $motivo, $pv_id, $venta_id);
    $stmt_u->execute();
    $stmt_u->close();

    // 4. Recalcular el saldo del empleado excluyendo los anulados
    ...
    if (function_exists('registrarLog')) {
        ...
        $datosDespues['anulado'] = 1;
        $datosDespues['motivo_anulacion'] = $motivo;
    ...
    $mysqli->commit();
    echo json_encode(["status" => "ok", "msg" => "Comprobante anulado y consumos actualizados."]);
